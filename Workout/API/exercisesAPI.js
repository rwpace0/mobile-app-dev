...
class ExercisesAPI extends APIBase {
      ...
      async (isInitialSync = false) => {
        ...
        if (isInitialSync) {
          ...
          if (localCount.count === 0) {
            console.log(
              "[ExercisesAPI] Local table empty, fetching initial data from server"
            );
            try {
          ...
          for (const exercise of pendingExercises) {
            try {
              ...
              if (syncPriority === "immediate") {
                console.log(
                  `[ExercisesAPI] Skipping immediate priority exercise ${exercise.exercise_id} - should already be synced`
                );
                continue;
              }
              ...
              const isUpdate =
                exercise.last_synced_at &&
                exercise.sync_status === "pending_sync";
              const method = isUpdate ? "PUT" : "POST";
              const url = isUpdate
                ? `${this.baseUrl}/${exercise.exercise_id}`
                : `${this.baseUrl}/create`;

              console.log(
                `[ExercisesAPI] ${
                  isUpdate ? "Updating" : "Creating"
                } exercise ${exercise.exercise_id} via ${method} ${url}`
              );

              // Prepare exercise data for sync - parse secondary_muscle_groups if it's a string
              const exerciseForSync = { ...exercise };
              if (
                exerciseForSync.secondary_muscle_groups &&
                typeof exerciseForSync.secondary_muscle_groups === "string"
              ) {
                try {
                  exerciseForSync.secondary_muscle_groups = JSON.parse(
                    exerciseForSync.secondary_muscle_groups
                  );
                } catch {
                  exerciseForSync.secondary_muscle_groups = [];
              ...
              if (isUpdate) {
                ...
                console.log(
                  `[ExercisesAPI] Exercise ${exercise.exercise_id} updated successfully`
                );
              ...
              console.error(
                `[ExercisesAPI] Failed to sync exercise ${exercise.exercise_id}:`,
                error
              );
            }
          }
          ...
          for (const exercise of exercisesForDeletion) {
            try {
              ...
              if (exercise.last_synced_at) {
                ...
                console.log(
                  `[ExercisesAPI] Exercise ${exercise.exercise_id} was never synced to server, skipping server deletion`
                );
              }
              ...
              if (error.response && error.response.status === 404) {
                ...
                console.log(
                  `[ExercisesAPI] Exercise ${exercise.exercise_id} not found on server (404) - deleting locally`
                );
                ...
                console.error(
                  `[ExercisesAPI] Failed to delete exercise ${exercise.exercise_id} from server:`,
                  error
                );
  ...
  // Helper to parse secondary_muscle_groups from SQLite JSON string
  _parseSecondaryMuscleGroups(exercise) {
    if (exercise && exercise.secondary_muscle_groups) {
      if (typeof exercise.secondary_muscle_groups === "string") {
        try {
          exercise.secondary_muscle_groups = JSON.parse(
  ...
  async storeLocally(exercise, syncStatus = "synced") {
    ...
    // Handle secondary_muscle_groups serialization
    let processedExercise = { ...exercise };
    if (exercise.secondary_muscle_groups) {
      // If it's already a string, keep it; if it's an array, stringify it
      processedExercise.secondary_muscle_groups = Array.isArray(
        exercise.secondary_muscle_groups
      )
    ...
    const result = await this.storage.storeEntity(processedExercise, {
      table: "exercises",
      fields: [
        "exercise_id",
        "name",
  ...
  async getExercises() {
    try {
      ...
      return this.handleOfflineFirst("exercises:all", async () => {
        ...
        // Parse secondary_muscle_groups for all exercises
        const parsedExercises = exercises.map((ex) =>
          this._parseSecondaryMuscleGroups(ex)
        );
  ...
  async getExerciseById(exerciseId) {
    try {
      ...
      return this.handleOfflineFirst(`exercise:${exerciseId}`, async () => {
        ...
        return exercise ? this._parseSecondaryMuscleGroups(exercise) : null;
      });
    } catch (error) {
  ...
  async createExercise(
    ...
    try {
      ...
      const exerciseData = {
        ...
        equipment,
        muscle_group,
        secondary_muscle_groups: Array.isArray(secondary_muscle_groups)
          ? secondary_muscle_groups
          : [],
      ...
      if (syncImmediately) {
        try {
          ...
          console.warn(
            "[ExercisesAPI] Immediate sync failed, will sync later:",
            syncError
          );
  ...
  async updateExercise(
    ...
    try {
      ...
      // Ensure secondary_muscle_groups is properly handled
      const processedSecondaryMuscles =
        secondary_muscle_groups !== undefined
          ? Array.isArray(secondary_muscle_groups)
            ? JSON.stringify(secondary_muscle_groups)
            : secondary_muscle_groups
          : existingExercise.secondary_muscle_groups;

      const updateData = {
        name: name.trim(),
        equipment,
        muscle_group,
        secondary_muscle_groups: processedSecondaryMuscles,
        instruction: instruction?.trim() || null,
        updated_at: now,
        sync_priority: syncImmediately ? "immediate" : "background",
      };
      ...
      await this.db.execute(
        ...
        [
          ...
          updateData.secondary_muscle_groups,
  ...
  async deleteExercise(exerciseId) {
    try {
      ...
      try {
        ...
        if (
          existingExercise.sync_status === "pending_sync" ||
          !existingExercise.last_synced_at
        ) {
      ...
      if (exercise.sync_status === "pending_delete") {
        try {
          ...
          if (exercise.last_synced_at) {
            ...
            console.log(
              `[ExercisesAPI] Exercise ${exerciseId} was never synced to server, skipping server deletion`
            );
          }
          ...
          await this.db.execute("COMMIT");
          console.log(
            `[ExercisesAPI] Successfully deleted exercise ${exerciseId} from local database`
          );
        } catch (error) {
          if (error.response && error.response.status === 404) {
            // Exercise doesn't exist on server, just delete locally
            console.log(
              `[ExercisesAPI] Exercise ${exerciseId} not found on server (404) - deleting locally`
            );
            ...
            await this.db.execute("ROLLBACK");
            console.error(
              `[ExercisesAPI] Failed to delete exercise ${exerciseId}:`,
              error
            );
            throw error;
          }
  ...
  async _fetchFromServer() {
    ...
    try {
      ...
      console.error(
        "[ExercisesAPI] Failed to fetch exercises from server:",
        error
      );
      throw error;
    }
  ...
  async syncExerciseWithMedia(exerciseId) {
    try {
      ...
      let secondaryMuscleGroups = exercise.secondary_muscle_groups;
      if (secondaryMuscleGroups && typeof secondaryMuscleGroups === "string") {
        try {
          secondaryMuscleGroups = JSON.parse(secondaryMuscleGroups);
        } catch {
      ...
      return {
        ...exercise,
        exercise_id: isUpdate
          ? exerciseId
          : response.data.exercise_id || exerciseId,
        sync_status: "synced",
        last_synced_at: now,
      };
    } catch (error) {
      console.error(
        "[ExercisesAPI] Failed to sync exercise with media:",
        error
      );
      console.error(
        "[ExercisesAPI] Error details:",
  ...
  async getExerciseMedia(exerciseId) {
    try {
      ...
      console.error(
        `[ExercisesAPI] Failed to get exercise media for ${exerciseId}:`,
        error
      );
      return null;
    }
  ...
  async downloadExerciseMedia(exerciseId, mediaUrl) {
    try {
      const { mediaCache } = await import("./local/MediaCache");
      return await mediaCache.downloadExerciseMediaIfNeeded(
        exerciseId,
        mediaUrl
      );
    } catch (error) {
